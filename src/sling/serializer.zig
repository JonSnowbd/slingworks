    ...
    pub fn toNode(tree: *Tree, value: anytype) *Node {
        ...
        switch (typeInfo) {
            ...
            .Union => {
                ...
                std.debug.panic("Failed to serialize {s}", .{@typeName(@TypeOf(value))});
            },
        }
    }
};

pub const NodeLiteral = union(enum) { String: []const u8, Bool: bool, Integer: i32, Float: f32, Null };
    ...
    pub fn into(self: *Node, ptr: anytype, owner: ?std.mem.Allocator) void {
        ...
        switch (typeInfo) {
            ...
            .Union => {
                ...
                std.debug.panic("Failed to deserialize type {s}\n", .{@typeName(T)});
            },
        }
    }
};
...
test "Basic Raw Deserialization" {
    ...
    // Struct:
    var playerDeserialized = into{ .health = undefined, .mana = undefined };
