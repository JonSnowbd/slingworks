...
pub const SceneRegister = struct {
...
pub var RegisteredScenes = std.StringHashMap(SceneRegister).init(sling.alloc);
...
pub fn scene(comptime T:type, comptime children: anytype) void {
    ...
    var entry = RegisteredScenes.getOrPut(@typeName(T)) catch unreachable;
